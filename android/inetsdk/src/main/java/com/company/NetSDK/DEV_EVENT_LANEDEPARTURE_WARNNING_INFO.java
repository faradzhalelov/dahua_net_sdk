 ...
 * \if ENGLISH_LANG
 *
 * @brief Corresponding to data block description of event type EVNET_IVS_LANEDEPARTURE_WARNNING(lane departure warnning)
 * \else
 * @brief 事件类型 EVNET_IVS_LANEDEPARTURE_WARNNING(车道偏移预警) 对应的数据块描述信息
 * \endif
 */
public class DEV_EVENT_LANEDEPARTURE_WARNNING_INFO implements Serializable {
    /**
     *
     */
    ...
    /**
     * \if ENGLISH_LANG
     * Departure Status, -1: unknown, 0: unbiased, 1: left biased, 2: right biased
     * \else
     * 偏移状态，-1:未知, 0:没偏, 1:左偏, 2:右偏
     * \endif
     */
    public int nDepartureStatus;

    /**
     * \if ENGLISH_LANG
     * The central deviation distance of the current lane, in meters
     * \else
     * 当前车道中央偏离距离，单位米
     * \endif
     */
    public float fDepartureDistance;

    /**
     * \if ENGLISH_LANG
     * The angle between the vehicle body and the lane line, in degrees
     * \else
     * 车身与车道线的角度，单位度
     * \endif
     */
    public float fDepartureAngle;
