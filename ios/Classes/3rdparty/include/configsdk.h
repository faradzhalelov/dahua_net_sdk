...
///@brief 梯控功能能力集
typedef struct tagNET_SUPPORT_CALLLIFT_FUNCTION
{
	BOOL								bWeeklyPlan;					/// 是否支持梯控周计划
	BOOL								bPublicOpenDayPlan;				/// 是否支持开放日计划
	BOOL								bPublicFloors;					/// 是否支持平台下发公共楼层
	BOOL								bOneClickControl;				/// 是否支持一键管控
	char								szReserved[64];					/// 预留字节
}NET_SUPPORT_CALLLIFT_FUNCTION;
...
typedef struct tagCFG_CAP_ACCESSCONTROL
{
	...
	BOOL								bSupportCallliftFunction;		/// 是否支持梯控功能能力集
	NET_SUPPORT_CALLLIFT_FUNCTION		stuSupportCallliftFunction;		/// 梯控功能能力集
	char								szResvered[920];				// 保留字段
...
///@brief 自定义OSD标题叠加
typedef struct tagCFG_CAP_CUSTOM_OSD_TITLE_INFO
{
	BOOL			bSupport;					 /// 是否支持自定义叠加,true 支持,false 不支持
	unsigned int	nTitleLine;					 /// 自定义标题默认行数
	unsigned int    nWebExtensionTitleLine;		 /// 自定义标题web显示允许扩展行数
	unsigned int    nMaxCharacterSizeTitleLine;	 /// 自定义标题每行最大可输入字符个数
	BOOL			bCustomTitleAlignmentOsd;	 /// 自定义标题对齐是否显示
	char			szReserved[508];			 /// 保留字节
} CFG_CAP_CUSTOM_OSD_TITLE_INFO;
...
typedef struct tagCFG_CAP_CUSTOM_OSD_INFO
{
	...
	CFG_CAP_CUSTOM_OSD_TITLE_INFO stuCustomOSDTitle;	/// 自定义OSD标题叠加
...
typedef struct tagCFG_VIDEO_FORMAT
{
	...
	int					nAiGOP;									// 图像组I帧间隔,1-150
	int					nSVCTLayer;								// SVC-T层数
	char				szCustomResolutionName[16];				// 默认分辨率名称
	int					nPriority;								// 编码优先级,0:画质优先(默认),  1:码率优先
	UINT				nQualityRange;							// 图像质量取值范围:6,100
	int					nLimitBitRate;							// 码流控制模式ABR下,瞬时码率上限,kbps为单位
	BOOL				bVirutualIFrameEnable;					// 码流控制模式ABR下,是否使能虚拟I帧功能
	int					nEncodeType;							// 0表示General,1表示SmartEncode,2表示Aicoding
	int					nQualityEx;								// 图像质量,范围1-nQualityRange
	bool				abAiGOP;								// nAiGOP字段是否有效
	bool				abSVCTLayer;							// nSVCTLayer字段是否有效
	bool				abPriority;								// nPriority字段是否有效
	bool				abQualityRange;							// nQualityRange字段是否有效
	bool				abLimitBitRate;							// nLimitBitRate字段是否有效
	bool				abVirutualIFrameEnable;					// bVirutualIFrameEnable字段是否有效
	bool				abEncodeType;							// nEncodeType字段是否有效
	bool				abCustomResolutionName;					// szCustomResolutionName字段是否有效
	BOOL				bUseQualityEx;							// 使用QualityEx字段表示图像质量
...
///@brief 视频浓缩能力
typedef struct tagCFG_SUPPORT_VIDEO_COMPRESS_ENCODECAP
{
	BOOL						bSupport[4];                                // 视频浓缩能力数组，4个元素，对应主码流、辅1、辅2、辅3
	char							szReserved[64];							// 保留字节
}CFG_SUPPORT_VIDEO_COMPRESS_ENCODECAP;
...
typedef struct tagCFG_ENCODECAP
{
	...
	int					 nMaxExtraStream;								// 最大辅码流路数
	int					nSupportVideoCompressNum;				// 视频浓缩能力元素数量
	CFG_SUPPORT_VIDEO_COMPRESS_ENCODECAP stuSupportVideoCompress[1024];
	char							szReserved[1020];							// 保留字节
}CFG_ENCODECAP;
...
typedef struct tagCFG_CROSSLINE_INFO
{
    ...
    int						nCorrelationTime;										/// 关联时间，单位：秒，范围：0-60。在绊线事件上报后在关联时间内触发打电话或者玩手机事件上报一个携带手机事件。 当值为0的时候，不生效
	UINT					nSensitivity;											/// 灵敏度，值越小灵敏度越低,取值1-10
	char					szReserved[1012];										/// 预留字节
} CFG_CROSSLINE_INFO;
...
typedef struct tagCFG_FACEANALYSIS_INFO
{
	...
	CFG_FACE_BEAUTIFICATION	stuFaceBeautification;									/// 人Lian美化
	UINT					nRecognizeDistance;										/// 人脸的识别距离，单位cm（设为0表示自动，识别距离不受限制）
	char                	szReserved[1020];                                       // 保留字段
...
typedef struct tagPEOPLE_STAT_REGIONS_INFO
{
	...
	char				szName[32];												// 区域名称(废弃，请使用szNameEx字段)
	...
	char				szNameEx[128];											// 区域名称,协议备注前63位有效
...
///@brief 无帽子检测
typedef struct tagNET_NO_HAT
{
	BOOL						bEnbale;				// 是否检测无帽子
	UINT						nTypeLegal;				// 检测类型,0-字段不存在,1-合规,2-不合规,3-未定义
	char                        szType[32];				// 无帽子类型
	UINT						nMinDuration;			// 最短持续时间,单位秒,默认值10,范围0-3600
	UINT						nReportInterval;		// 重复报警间隔,单位秒,0-600,等于0表示不重复报警,默认值30
	UINT						nSensitivity;			// 检测灵敏度,取值范围1-10,默认值5,灵敏度越高越容易检测
	UINT						nAlarmType;				// 关联规则中表示子规则是正向还是反向事件,0-字段不存在,1为反向事件,2为正向事件
	UINT						nAlarmOrder;			// 子规则的顺序，组合关联规则生效的先后顺序
	UINT						nAlarmOriented;			// 按规则,或目标报警,0-字段不存在,1为按目标报警,2为按规则报警
	char						szReserved[32];		// 保留字节
}NET_NO_HAT;
...
typedef struct tagNET_WORK_CLOTHES_DESCRIPTION
{
	...
	int					nWorkClothesDescriptionType;	/// 0:全部检测规则, 1:安全帽检测规则, 2:工作服检测规则, 3:工作马甲检测规则, 4:工作裤检测规则
	NET_NO_HAT			stuNoHat;						// 无帽子检测
	BOOL				bUseNoHat;						// 是否启用NoHat字段
	BYTE				bReserved[316];					// 保留字节
...
///@brief 加热器
typedef struct tagCFG_PTZ_HEATER
{
	BOOL					bSupport;					/// 是否支持加热器控制
	BOOL					bSupportAutoClose;			/// 是否支持自动关闭控制
	int						nSupportTypesNum;			/// 支持的加热器类型个数
	char					szSupportTypes[8][32];		/// 支持的加热器类型:"Total":整机加热, "Thermal":热成像镜头加热器, "Visual":可见光镜头加热器, "Demisting":除雾除冰加热器, "Ptz":云台加热器, "Shield":护罩加热器
	int						nModeNum;					/// 加热器模式个数
	char					szMode[8][32];				/// 加热器模式: "Close":关闭, "Auto":自动, "Manual":手动, "Time":"定时"
	BOOL					bSupportManualControl;		/// 是否支持手动控制
	int						nManualControlPeriod[2];	/// 手动控制运行时间范围[最小时间，最大时间]单位/分钟
	int						nHeaterTempLowRange[2];		/// 开启加热器温度范围[最小温度，最大温度], 单位：0.1摄氏度
	int						nHeaterTempHighRange[2];	/// 关闭加热器温度范围[最小温度，最大温度], 单位：0.1摄氏度
	char					szReserved[1020];			/// 预留字节
}CFG_PTZ_HEATER;
...
typedef struct tagCFG_PTZ_PROTOCOL_CAPS_INFO
{
	...
	CFG_PTZ_HEATER					stuPtzHeater;				/// 加热器能力
...
///@brief 自动远程出门
typedef struct tagNET_AUTO_REMOTE_OUT_DOOR_CHECK
{
	BOOL				bEnable;				/// 使能项
	UINT				nTime;					/// 自动远程出门时间段
	UINT				nHolidayScheduleID;		/// 远程验证假日计划
	char				szReserved[60];			/// 预留字段
}NET_AUTO_REMOTE_OUT_DOOR_CHECK;
...
typedef struct tagCFG_ACCESS_EVENT_INFO
{
	...
	BOOL				bAutoRemoteOutDoorCheck;						/// 自动远程出门使能
	NET_AUTO_REMOTE_OUT_DOOR_CHECK	stuAutoRemoteOutDoorCheck;			/// 自动远程出门
	char				szResvered[768];								// 保留字节
